...
labelconvert brodmannATLAS_in_FSL.mif $brodmann_path/../../BrodmannColorLUT.txt $brodmann_path/../../Brodmann_known_default.txt Brodmann_known_nodesFSL.mif
mkdir separated_Known_Brodmann
while IFS= read -r line || [ -n "$line" ]; do #captures also the last line,that has no newline at the end
    stringarray=($line)
    code=${stringarray[0]}
    label=${stringarray[1]}
    echo $code
...
do
  ...
  mrgrid bilateral/$area_nii regrid -template $path/functional_HCP/Template4regridding.mif -interp nearest bilateral/regridded/$area_nii
done;
