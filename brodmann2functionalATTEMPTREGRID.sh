...
labelconvert brodmannATLAS_in_FSL.mif $brodmann_path/../../BrodmannColorLUT.txt $brodmann_path/../../Brodmann_known_default.txt Brodmann_known_nodesFSLATTEMPT.mif
...
while IFS= read -r line || [ -n "$line" ]; do #caputers also the last line,that has no newline at the end
    ...
    if ! [[ -z "${code// }" ]];
    then
      mrcalc Brodmann_known_nodesFSL.mif $code -eq separated_Known_Brodmann/$label_ATTEMPT.nii
    fi
...
do
  ...
  area_nii=${i##*-}
  echo $counter ${area_nii%.*} >> $path/AtlasCollection/Brodmann/Brodmann_known_default_bilateral_ATTEMPT.txt
  counter=$((counter+1))
  symmetric_files=$(find *${i##*-}) ;#everything after last occurrence of -
  #echo joining ${symmetric_files[0]} ${symmetric_files[1]}
  mrcalc ${symmetric_files[0]} ${symmetric_files[1]} -max ATTEMPT_$area_nii
  mrgrid ATTEMPT_$area_nii regrid -template $path/functional_HCP/Template4regridding.mif -interp nearest ATTEMPT_regridded_$area_nii
done;
